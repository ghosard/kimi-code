...
| 命令 | 别名 | 说明 | 随时可用 |
| --- | --- | --- | --- |
| `/login` | — | 选择账号或平台并登录：OpenAI Codex 可选浏览器或 device-code OAuth，Kimi Code 使用 device-code OAuth，Kimi Platform 使用 API 密钥 | 否 |
...
| `/usage` | — | 显示 token 用量、提示缓存读取量与命中率、上下文占用以及配额信息 | 是 |
